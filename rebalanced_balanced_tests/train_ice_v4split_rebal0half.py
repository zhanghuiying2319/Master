...
class icedataset_train_uresize_rebal0half(torch.utils.data.Dataset):
  def __init__(self, thenumpyfile, transforms ):
    ...
    self.indsforeachclass = [ [] for _ in range(numcl) ]
    
    labels = np.zeros((c.shape[0],numcl), dtype = np.int32)
    for l in range(c.shape[0]):
      labels[l,:]=c[l]
      posind = np.flatnonzero(c[l])
      if len(posind)!=1:
        print('len(posind)!=1 ?!')
        exit()
      self.indsforeachclass[posind[0]].append(l)
      
    self.labels=torch.IntTensor(labels)
    
    
    counts = np.array([ 9087.0,
    239.0,
    201.0,
    1934.0,
    684.0,
    ...
    counts2[0]=counts2[0]/3.0
    
    self.classprob = counts2 /np.sum(counts2)
  ...
  def __getitem__(self,idx0):
    ...
    #pad
    #select index
    chosenclass = self.rng.choice( range(self.numcl), size=1, replace=False, p=None)

    idx = self.rng.choice( self.indsforeachclass[chosenclass[0]], size=1, replace=False, p=None)
    idx=idx[0]
    #print( chosenclass[0], idx, self.labels[idx, chosenclass[0]])
...
def runstuff():
  ...
  dataset_trainval = icedataset_train_uresize_rebal0half('./test_withBoundaries_new_Julie.npy', transforms = data_transforms['train']  )
  dataset_test = icedataset_train_uresize_rebal0half('./test_withBoundaries_new_Julie.npy', transforms =  data_transforms['test'])
  ...
  for cvind in range(config['numcv']):
    ...
    dataloader_train = torch.utils.data.DataLoader(dataset = dataset_trainval, batch_size= config['batchsize_train'], shuffle=False, sampler=sampler_train, batch_sampler=None, num_workers=1, collate_fn=None)
    ...
    dataloader_val = torch.utils.data.DataLoader(dataset = dataset_trainval, batch_size= config['batchsize_val'], shuffle=False, sampler=sampler_val, batch_sampler=None, num_workers=1, collate_fn=None)
