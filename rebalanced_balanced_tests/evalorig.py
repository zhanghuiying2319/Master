...
def runstuff():
  ...
  for cvind in range(numcv):
    cwacctmp=np.load(os.path.join(savept,'noavg_cwacc_outercv{:d}.npy'.format(cvind)) )
    globalacctmp=np.load(os.path.join(savept,'noavg_globalacc_outercv{:d}.npy'.format(cvind)) )
    
    #confusion_matrixtmp=np.load(os.path.join(savept,'noavg_confusion_matrix_outercv{:d}.npy'.format(cvind)))
    
    globalacc+=globalacctmp / float(numcv)
    cwacc+=cwacctmp / float(numcv)
    
    conf = np.load(    os.path.join(savept,'noavg_confusion_matrix_outercv{:d}.npy'.format(cvind)))
    confusion+=conf
